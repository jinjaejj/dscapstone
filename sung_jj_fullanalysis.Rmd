---
title: "STAT 219 Final Project Report - Full Analysis"
author: "JJ Sung"
date: "5/28/2021"
output: pdf_document
---
...
#Extract coordinates
#us_vacc_apr_coord <- data.frame(na.omit(select(us_vaccSites_apr, X, Y, name)))
#us_vacc_may_coord <- data.frame(na.omit(select(us_vaccSites_may, X, Y, name)))
#us_vacc_dec_coord <- data.frame(na.omit(select(us_vaccSites_dec, X, Y, name)))
...
#county_fortify$med_income <- as.numeric(county_fortify$med_income)
us_map + geom_polygon(data = county_fortify, 
                      aes(x = long, y = lat, group = group, 
...
#total female population percentage by county
us_map + geom_polygon(data = county_fortify, 
...
#total POC population percentage by county
us_map + geom_polygon(data = county_fortify, 
                      aes(x = long, y = lat, group = group, 
...
## Initial full models
```{r linreg}
county_shape_ref <- county_shape@data[-which(is.na(county_shape@data$num_vacc_sites)),]
...
vif(lin_reg) #all values smaller than 2
...
cv.fo.model1 <- train(num_vacc_sites~med_income+sum_tot_pop+perc_poc+perc_female, data = int_var, method = "lm", trControl = ctrl)
cv.fo.model2 <- train(num_vacc_sites~sum_tot_pop+perc_female, data = int_var, method = "lm", trControl = ctrl)
cv.bic.int.model<- train(num_vacc_sites ~ med_income+sum_tot_pop+perc_poc+perc_female +med_income:sum_tot_pop+med_income:perc_poc+med_income:perc_female+sum_tot_pop:perc_poc +sum_tot_pop:perc_female, data = int_var ,method="lm",trControl=ctrl)
...
Spatial Lag Model
```{r slag}
vacc_lag <- lagsarlm(log(num_vacc_sites) ~ med_income + sum_tot_pop + perc_female + perc_poc, data = county_shape@data, listw = W_list, zero.policy = TRUE)
summary(vacc_lag) #AIC: 6487.1
```

CAR
```{r car}
vacc_car <- spautolm(log(num_vacc_sites) ~ med_income + sum_tot_pop + perc_female + perc_poc, data = county_shape@data, family = "CAR", listw = W_list, zero.policy = TRUE)
...
#county_shape <- county_shape[-which(county_shape$ALAND==0),]

#California
#us_vaccSites_pp <- us_vaccSites %>% as.data.frame()
...
ca_one <- st_transform(st_union(st_as_sf(ca_poly)), crs = CRS("+init=epsg:2225"))
